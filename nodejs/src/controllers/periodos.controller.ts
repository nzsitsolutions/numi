...
export default {
    getList: async (req: any, res: any) => {
        ...
        res.status(200).json(list((data ?? []).map((p: any) => periodosService.calcularVOs(p))));
    },
    getOne: async (req: any, res: any) => {
        const { data, error } = await periodosService.firstOrDefaultAsync(
            Number(req.params.anio),
            Number(req.params.mes),
        );

        if (error) return res.status(500).json(error);
        if (!data || data.length === 0) return res.status(404).json({ message: "periodo no encontrado" });

        res.status(200).json(single(periodosService.calcularVOs(data[0])));
    },
    getResumen: async (req: any, res: any) => {
        const { data, error } = await resumenService.getResumenMensualAsync(
            Number(req.params.anio),
            Number(req.params.mes),
        );
    ...
    updateTipoCambio: async (req: any, res: any) => {
        const { data, error } = await periodosService.updateTipoCambioAsync(
            Number(req.params.anio),
            Number(req.params.mes),
            req.body.tipoCambio,
        );

...
export default {
    getResumenMensualAsync: async (anio, mes) => {
        ...
        if (primerError)
            return { data: null, error: { message: primerError.message } };
        ...
        const tarjetaPorGasto = new Map();
        for (const t of tarjetasData) {
            if (t.gasto_id)
                tarjetaPorGasto.set(t.gasto_id, t);
        }
        const tieneTarjetaReal = (gastoId) => {
        ...
        const cuotaMensual = (g) => {
            if (String(g.tipo).toLowerCase() === "fijo")
                return g.monto_ars + g.monto_usd * tipoCambio;

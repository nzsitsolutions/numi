...
export function ImportacionesPage() {
  const { pendingMovements, confirmMovement, discardMovement } = useNumi()
  ...
  const handleFileUpload = useCallback((type: 'pdf' | 'excel') => {
    // Simulate file upload and parsing
    setImportResults({
      type: type === 'pdf' ? 'NaranjaX PDF' : 'BBVA Excel',
      newCount: Math.floor(Math.random() * 10) + 3,
  ...
  const handleGoogleDriveSync = useCallback((source: string) => {
    setImportResults({
      type: `${source} (Drive)`,
      newCount: Math.floor(Math.random() * 8) + 2,
      duplicateCount: Math.floor(Math.random() * 2),
      errorCount: 0
    })
          ...
          <Card className="bg-card border-border">
            ...
            <CardContent className="space-y-3">
              ...
              >
                ...
                <p className="text-sm text-muted-foreground">
                  Arrastrá tu PDF o hacé click para subir
                </p>
              ...
              <Button 
                ...
                onClick={() => handleGoogleDriveSync('NaranjaX')}
              >
          ...
          <Card className="bg-card border-border">
            ...
            <CardContent className="space-y-3">
              ...
              >
                <Upload className="h-8 w-8 text-muted-foreground mx-auto mb-2" />
                <p className="text-sm text-muted-foreground">
                  Arrastrá tu Excel o hacé click para subir
                </p>
                <p className="text-xs text-muted-foreground mt-1">
              ...
              <Button 
                variant="outline" 
                className="w-full gap-2"
                onClick={() => handleGoogleDriveSync('BBVA')}
              >
                <Cloud className="h-4 w-4" />
                Sincronizar desde Google Drive
              </Button>
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="flex items-center gap-2 mt-4 pt-3 border-t border-border">
                        <Button 
                          ...
                          onClick={() => confirmMovement(movement.id)}
                        >
                        ...
                        <Button 
                          variant="outline" 
                          className="flex-1 gap-1"
                          onClick={() => discardMovement(movement.id)}
                        >
                          <X className="h-4 w-4" />
                          Descartar
                        </Button>
